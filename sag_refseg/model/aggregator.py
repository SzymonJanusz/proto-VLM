"""Perceiver-style slot aggregator (ported from kdwonn/SaG)."""
import torch
...
from .attention import (
    Attention, PreNorm, PostNorm, FeedForward, ThinFeedForward,
    Queries, cache_fn, cache_fn2
)
    ...
    def __init__(
        ...
        *,
        ...
        self.queries = Queries(
            self.query_type, self.num_latents, self.latent_dim,
        ...
        cross_attn_config = {
            'heads': cross_heads, 'dim_head': cross_dim_head, 'dropout': attn_dropout,
            'attn_type': cross_attn_type, 'more_dropout': more_dropout,
            'xavier_init': xavier_init, 'gumbel': gumbel_attn
        }
        cross_ff_config = {
            'dropout': ff_dropout, 'activation': activation, 'mult': ff_mult,
            'more_dropout': more_dropout, 'xavier_init': xavier_init
        }
        latent_attn_config = {
            'heads': latent_heads, 'dim_head': latent_dim_head, 'dropout': attn_dropout,
            'more_dropout': more_dropout, 'xavier_init': xavier_init
        }
        latent_ff_config = {
            'dropout': ff_dropout, 'activation': activation, 'mult': ff_mult,
        ...
        get_cross_attn, get_cross_ff, get_latent_attn, get_latent_ff, get_pre_self_attn, get_pre_self_ff = map(
            cache_fn, (get_cross_attn, get_cross_ff, get_latent_attn, get_latent_ff, get_pre_self_attn, get_pre_self_ff)
        )
        ...
        self.last_layer = nn.Sequential(
            nn.LayerNorm(latent_dim) if last_ln and not norm_post else nn.Identity(),
