"""Positional encodings (ported from kdwonn/SaG)."""
import torch
...
def build_position_encoding(dim, pos_type, axis):
    N_steps = dim // 2
    if pos_type in ('sine',):
        position_embedding = PositionEmbeddingSine1D(dim, normalize=True) if axis == 1 \
            else PositionEmbeddingSine2D(N_steps, normalize=True)
    elif pos_type in ('learned',):
        position_embedding = PositionEmbeddingLearned1D(dim) if axis == 1 \
            else PositionEmbeddingLearned2D(N_steps)
