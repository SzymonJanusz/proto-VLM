"""Reconstruction decoders (ported from kdwonn/SaG)."""
import torch
...
class MlpDecoder(nn.Module):
    def __init__(self, num_patches, slot_dim, feat_dim, normalizer='softmax', self_attn=False, pos_enc='learned'):
        super().__init__()
...
class TransformerDecoder(nn.Module):
    def __init__(self, num_patches, slot_dim, feat_dim, normalizer='softmax', pos_enc='learned', self_attn=False):
        super().__init__()
        self.pos_emb = build_position_encoding(slot_dim, pos_enc, axis=2)
        self.num_patches = num_patches
        self.alpha_holder = nn.Identity()
