"""Image and text encoders (ported from kdwonn/SaG)."""
import dataclasses
import torch
...
def get_img_backbone(arch, pretrained, num_layers, img_size):
    if arch == 'resnext_wsl':
        ...
        if arch in default_cfgs:
            raw = default_cfgs[arch]
            default_cfg = dataclasses.asdict(raw) if dataclasses.is_dataclass(raw) else dict(raw)
        else:
            default_cfg = dict(
                pretrained=False,
                num_classes=1000,
...
class SetPredictionModule(nn.Module):
    def __init__(self, num_embeds, axis, pos_enc, args):
...
class ImageTextEncodersRecon(nn.Module):
    ...
    def forward(self, images, sentences, txt_len):
        with torch.cuda.amp.autocast(enabled=self.amp):
            ...
            img_slot, img_feat, img_attn, img_residual = self.img_enc(
                Variable(images), txt_emb=txt_emb if self.slot_cond else None
            )
...
class EncoderImage(nn.Module):
    def __init__(self, args):
        ...
        self.img_backbone, self.local_feat_dim = get_img_backbone(
            args.img_backbone, True, args.num_layers, args.crop_size
        )
        ...
        if args.agg_xavier_init:
            self.set_pred_module.agg.apply(
                lambda m: nn.init.xavier_normal_(m.weight) if hasattr(m, 'weight') and m.weight.dim() > 1 else None
            )
...
class EncoderTextBERT(nn.Module):
    ...
    def forward(self, x, lengths):
        ...
        bert_emb = bert_emb[0]
        cap_len = lengths
        bert_emb_out = self.residual_connection(bert_emb, bert_emb[:, 0], cap_len)
        local_cap_emb = self.linear(bert_emb)
        out = self.residual_connection(local_cap_emb, local_cap_emb[:, 0], cap_len)
