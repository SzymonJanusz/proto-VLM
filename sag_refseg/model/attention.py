"""Attention mechanisms (ported from kdwonn/SaG)."""
...
class Attention(nn.Module):
    ...
    def norm_attn(self, sim, is_gumbel=False, gumbel_tau=1):
        if self.gumbel or is_gumbel:
            norm_fn = (lambda logits, dim: gumbel_softmax(logits, dim=dim, tau=gumbel_tau)) \
                if self.training else hard_softmax
        else:
            norm_fn = F.softmax
    ...
    def __init__(self, query_type, num, dim, cascade_factor=None, var_scale=0):
        ...
        if self.query_type == 'query':
            ...
            self.slots_mu = nn.init.xavier_uniform_(
                nn.Parameter(torch.randn(1, 1, dim)), gain=nn.init.calculate_gain("linear"))
            self.slots_log_sigma = nn.init.xavier_uniform_(
                nn.Parameter(torch.randn(1, 1, dim)), gain=nn.init.calculate_gain("linear"))
        elif self.query_type == 'entity':
            num_dist = self.num // self.cascade_factor
            self.slots_mu = nn.Parameter(nn.init.xavier_uniform_(
                torch.randn(1, num_dist, dim), gain=nn.init.calculate_gain("linear")))
            self.slots_log_sigma = nn.init.xavier_uniform_(
                torch.randn(1, num_dist, dim), gain=nn.init.calculate_gain("linear"))
            self.slots_log_sigma = nn.Parameter(
                self.slots_log_sigma + torch.log(torch.tensor(self.scale)))
        else:
            raise NotImplementedError
