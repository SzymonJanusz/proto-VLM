"""Cross-modal alignment loss (ported from kdwonn/SaG)."""
import torch
...
class CMA_Loss(nn.Module):
    ...
    def contrastive_loss(self, sim, gt, neg_gt=None):
    ...
    def triplet_loss(self, sim, gt, neg_gt=None):
        assert self.mining in ['hard', 'semi_hard', 'mean']
        ap = sim[gt].reshape(sim.shape[0], 1)
        an = sim[~gt].reshape(sim.shape[0], sim.shape[1] - 1)
        if self.mining == 'mean':
            loss = (self.margin + an - ap).clamp(min=0.0).mean()
            ...
            loss = (self.margin + an - ap).clamp(min=0.0).max(dim=-1)[0].mean()
        else:
            raise NotImplementedError
        return loss
...
class CMA_Loss_Fast(CMA_Loss):
    def __init__(self, margin, criterion, mining='hard', detach_target=False,
                 ...
                 cm_i_weight=1, size_p_loss=None, size_p_weight=0):
        super().__init__(
            margin, criterion, mining, detach_target, detach_img_target,
            i_t_loss, i_t_weight, temperature, cm_i_weight, size_p_loss, size_p_weight,
        )

    def pad_txts(self, txt_emb, txts_per_img):
        txt_chunks = torch.split(txt_emb, txts_per_img.tolist(), dim=0)
        padded_chunks = nn.utils.rnn.pad_sequence(
            sequences=txt_chunks, batch_first=True, padding_value=0)
        pad_mask = repeat(
            torch.arange(padded_chunks.shape[1]).cuda(), 'l -> repeat l', repeat=len(txts_per_img))
